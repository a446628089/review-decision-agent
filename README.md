...
自研 **Agent Harness 约束层**，通过装饰器包裹业务节点、**ContextVar** 传递运行上下文，提供预算控制、熔断、指数退避重试与 **Pydantic** 输出校验能力，覆盖**成本、熔断、重试、输出校验 4 类稳定性治理**。运行监控界面展示步骤、耗时、Token 消耗与估算成本，便于定位执行问题。
...
本地启动后端后，可访问 [FastAPI 交互式接口文档](http://localhost:8787/docs)，查看接口参数并发送测试请求。

## 使用说明

- 项目以录音上传、转写和决策检索为主要使用流程。
- Mock 转写使用内置示例语料；识别实际录音需按配置说明启用 DashScope ASR 与 OSS。
- 纪要生成、决策抽取与 RAG 问答需要可用的模型及向量服务。
- 较长的会议文本建议按议题分段处理；当前决策检测读取输入前 8,000 个字符，长文本压缩可能遗漏细节。
